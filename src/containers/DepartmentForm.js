...
const DepartmentForm = React.memo((props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      let res;
      if (id) {
        res = await api.updateResource('departments', id, {
          name: values.name,
        });
      } else {
        res = await api.createResource('departments', {
          name: values.name,
        });
      }
  ...
  return (
    <Container>
      ...
      <Typography variant="h5" component="h4" gutterBottom>
        {id ? 'Cập nhật phòng ban' : 'Thêm mới phòng ban'}
      </Typography>
              ...
              <Grid container>
                <Grid container className={classes.groupInput}>
                  <Grid item xs={12} md={3}>
                    <Typography variant="subtitle1">Tên phòng ban</Typography>
                  </Grid>
